    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-serif text-sm text-foreground leading-snug mb-0.5">
            {product.name}
          </h3>
...
export default function Reserve() {
  ...
  const [windowLoaded, setWindowLoaded] = useState(
    () =>
      readCache(BAKE_URL, DATA_TTL) !== undefined &&
      readCache(SETTINGS_URL, DATA_TTL) !== undefined,
  );
  ...
  useEffect(() => {
    ...
    function checkDone() {
      if (bwDone && sDone) setWindowLoaded(true);
    }
    revalidate<BakeWindow | null>(
      BAKE_URL,
      (d) => {
        setBakeWindow(d);
        bwDone = true;
  ...
  const BAKE_DATE = bakeWindow
    ? formatBakeDate(bakeWindow.bakeDate)
    : "Coming Soon";
  const isBakeFuture = bakeWindow
    ? new Date(bakeWindow.bakeDate) > new Date()
    : true;
  ...
  const canProceed =
    cartHasItems && !!(name.trim() && phone.trim() && address.trim());

  const cartLines = cartItems.map(
    (p) =>
      `• ${cart[p.id]}× ${p.name} @ ₹${p.price.toLocaleString("en-IN")} = ₹${((cart[p.id] ?? 0) * p.price).toLocaleString("en-IN")}`,
  );
  ...
  const upiUrl = upiId
    ? `upi://pay?pa=${encodeURIComponent(upiId)}&pn=Wandering%20Cocos&am=${cartTotal}&tn=Pre-order%20${encodeURIComponent(BAKE_DATE)}&cu=INR`
    : "";

  function scrollToOrder() {
    if (!orderSectionRef.current) return;
    const top =
      orderSectionRef.current.getBoundingClientRect().top +
      window.scrollY -
      100;
    ...
    <div className="min-h-screen bg-background flex flex-col selection:bg-accent/20">
      ...
      <main className="flex-grow">
            ...
            >
              {siteMode === "popup" ? (
                ...
                <>
                  ...
                  <BakeDateDisplay date={BAKE_DATE} dark />
                </>
              )}
            </motion.span>
        ...
        <section
          ref={orderSectionRef}
          id="preorder-section"
          className="px-6 md:px-14 lg:px-20 py-16 max-w-7xl mx-auto"
        >
          <div
            className={`grid grid-cols-1 gap-14 lg:gap-24 ${siteMode === "bake_day" ? "lg:grid-cols-2" : ""}`}
          >
                ...
                {!windowLoaded ? (
                  <div className="space-y-3">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                      <div
                        key={i}
                        className="py-4 flex gap-5 items-start animate-pulse"
                      >
            ...
            >
              {!siteModeLoaded ? (
                ...
                <div
                  className="border border-border/30 p-8 text-center"
                  style={{ background: "hsl(38 25% 97%)" }}
                >
                    ...
                    >
                      ...
                      {!windowLoaded ? (
                        <div className="space-y-3 mb-8">
                          {[1, 2, 3].map((i) => (
                            <div
                              key={i}
                              className="h-24 animate-pulse rounded bg-foreground/5"
                            />
                          ))}
                        </div>
                      ) : (
                      ...
                      <AnimatePresence>
                        {cartHasItems && (
                          ...
                          >
                            <div className="mb-8">
                              <p className="text-[10px] tracking-[0.22em] uppercase font-medium text-[#2D2926] mb-4">
                                Delivery details
                              </p>
                              <div className="space-y-3">
                                <div>
                                  <label className="text-[10px] tracking-[0.18em] uppercase text-[#2D2926] block mb-1">
                                    Full name
                                  </label>
                                  <input
                                    type="text"
                            ...
                            {/* Order summary */}
                            <div
                              className="border border-border/35 px-5 py-4 mb-6"
                              style={{ background: "hsl(38 25% 97%)" }}
                            >
                              <p className="text-[9px] tracking-[0.25em] uppercase font-medium text-[#2D2926] mb-3">
                                Order summary
                              </p>
                              ...
                              >
                                <p className="text-[10px] tracking-[0.15em] uppercase font-medium text-[#2D2926]">
                                  Total
                                </p>
                            ...
                            {!canProceed && (
                              <p className="text-[10px] text-[#2D2926] text-center">
                      ...
                      <div className="mb-8 pb-8 border-b border-border/25">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <p className="text-sm font-serif text-foreground">
                            Pre-order sent on WhatsApp
                          </p>
                        </div>
                        <p className="text-xs text-[#2D2926] leading-relaxed pl-8">
                          Your pre-order for {BAKE_DATE} has been sent to us.
                          We'll confirm your slot once we receive payment.
                        </p>
                      </div>

                      <div className="mb-8">
                        <p className="text-[10px] tracking-[0.28em] uppercase font-medium text-[#2D2926] mb-5">
                        ...
                        >
                            ...
                            {cartItems.map((p) => (
                              ...
                              >
                                ...
                                <span className="text-xs font-medium text-foreground">
                                  ₹
                                  {((cart[p.id] ?? 0) * p.price).toLocaleString(
                                    "en-IN",
                                  )}
                                </span>
                              </div>
                          ...
                          <div
                            className="flex items-baseline justify-between pt-3 mb-5"
                            style={{
                          ...
                          {upiId && (
                            <div className="flex flex-col gap-3">
                              <div>
                                <p className="text-[9px] tracking-[0.22em] uppercase text-[#2D2926] mb-1">
                                  UPI ID
                                </p>
                        ...
                        <p className="text-[10px] text-[#2D2926] leading-relaxed">
                          Once we confirm receipt of payment, we'll send you a
                          WhatsApp confirmation. Delivery on bake day.
                        </p>
                      </div>
