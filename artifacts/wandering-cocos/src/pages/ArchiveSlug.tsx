        ...
        {[1, 2, 3, 4, 5].map(star => (
          <button key={star} onClick={() => handleClick(star)} onMouseEnter={() => setHovering(star)}
            ...
            <svg width="16" height="16" viewBox="0 0 24 24"
              ...
              style={{ color: star <= active ? "hsl(150 40% 28%)" : "#2D2926", transition: "all 0.1s ease" }}>
      ...
      <div className="flex items-baseline gap-2">
        {voteCount > 0 ? (
          <>
            ...
            <span className="text-[10px] tracking-[0.12em] uppercase" style={{ color: "#0F2419" }}>
              {voteCount} {voteCount === 1 ? "vote" : "votes"}
            </span>
          </>
        ) : (
          <span className="text-[10px] tracking-[0.12em] uppercase" style={{ color: "#0F2419" }}>Be the first to rate</span>
        )}
        {voted && (
      ...
      <div className="flex gap-6 py-7" style={{ borderBottom: "1px solid rgba(15,36,25,0.1)" }}>
        {imageSrc ? (
          ...
          <div className="flex-shrink-0 rounded-sm flex items-center justify-center"
            ...
            <span className="font-serif italic" style={{ fontSize: "1.4rem", color: "#2D2926" }}>
              {String(index + 1).padStart(2, "0")}
            </span>
          </div>
        )}
        <div className="flex-grow min-w-0">
          <p className="text-[9px] tracking-[0.22em] uppercase font-medium mb-1" style={{ color: "#0F2419" }}>
            {String(index + 1).padStart(2, "0")}
          </p>
          <h3 className="font-serif font-medium leading-snug" style={{ fontSize: "1.05rem", color: "#0f2419" }}>{item.name}</h3>
          {item.description && (
            <p className="mt-1 leading-relaxed" style={{ fontSize: "0.78rem", color: "#0F2419" }}>{item.description}</p>
          )}
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "hsl(38 25% 96%)" }}>
      ...
      <main className="flex-grow max-w-3xl mx-auto w-full px-6 md:px-10 pt-20 pb-24">
        ...
        {loading ? (
          ...
          <p className="font-serif italic" style={{ color: "#0F2419" }}>Bake day not found.</p>
        ) : (
          <>
            <div className="mb-12">
              <p className="text-[9px] tracking-[0.36em] uppercase font-medium mb-4" style={{ color: "#0F2419" }}>
              ...
              <div className="flex items-end justify-between gap-5 pb-5" style={{ borderBottom: "2px solid #0f2419" }}>
                <div>
                  ...
                  <p className="text-[10px] tracking-[0.18em] uppercase" style={{ color: "#0F2419" }}>{formattedDate}</p>
                </div>
                {launch.notes && (
                  <p className="font-serif italic text-right flex-shrink-0" style={{ fontSize: "0.82rem", color: "#0F2419", maxWidth: "200px" }}>
                    {launch.notes}
                  </p>
                )}
              </div>
            ...
            {launch.items.length === 0 ? (
              <p className="font-serif italic" style={{ color: "#0F2419" }}>Items will be added here soon.</p>
            ) : (
              <div>
