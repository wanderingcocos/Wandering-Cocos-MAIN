...
function ItemsPanel({
  ...
  return (
    <div className="mt-4 ml-0 border-t border-border/25 pt-4">
      ...
      <p className="text-[9px] tracking-[0.28em] uppercase font-medium text-[#2D2926] mb-3">
        ...
        <div className="space-y-1.5 mb-4">
          {items.map((item, i) => (
            <div key={item.id} className="flex items-start gap-3 group">
              <span className="text-[9px] text-[#2D2926] font-medium w-4 mt-1 flex-shrink-0">
                ...
                <div className="flex-1 flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-[10px] tracking-widest uppercase px-3 h-7 bg-accent text-accent-foreground"
                    >
                      Save
                    </button>
                    <button
                      onClick={() => setEditItemId(null)}
                      className="text-[10px] tracking-widest uppercase px-3 h-7 border border-border/40 text-[#2D2926]"
                    >
                      Cancel
                    </button>
                  </div>
...
function BakeWindowsTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<BakeWindow[]>(`${API}/admin/bake-windows`, token);
    ...
    <div>
      ...
      <AnimatePresence>
        {creating && (
          ...
          >
            <p className="text-[10px] tracking-[0.25em] uppercase text-[#2D2926] mb-5">
              New Bake Window
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-[10px] tracking-widest uppercase text-[#2D2926] block mb-1">
                  Label
                </label>
                <input
                  value={form.label}
                  ...
                  placeholder="e.g. Weekend Drop #3"
              ...
              <div>
                ...
                <input
                  ...
                  value={form.bakeDate}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, bakeDate: e.target.value }))
                  }
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  value={form.boxPrice}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, boxPrice: Number(e.target.value) }))
                  }
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={saving || !form.label || !form.bakeDate}
      ...
      {loading ? (
        ...
        <p className="text-xs text-[#2D2926]">
          No bake windows yet. Create one above.
        </p>
      ) : (
            ...
            <div key={w.id} className="py-5">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#2D2926]">
                    {new Date(w.bakeDate + "T00:00:00").toLocaleDateString(
                      "en-IN",
                  ...
                  {w.notes && (
                    <p className="text-xs text-[#2D2926] mt-0.5 italic">
                      {w.notes}
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {editId === w.id ? (
                    <>
                      ...
                      <button
                        onClick={() => handleStatusUpdate(w.id)}
                        className="text-[10px] tracking-widest uppercase px-3 h-8 bg-accent text-accent-foreground"
                      >
                        Save
                      </button>
                      <button
                        onClick={() => setEditId(null)}
                        className="text-[10px] tracking-widest uppercase px-3 h-8 border border-border/50 text-[#2D2926]"
                      >
                        ×
                      </button>
                    </>
                    ...
                    <>
                      ...
                      <button
                        ...
                        disabled={
                          archiveStatus[w.id] === "loading" ||
                          archiveStatus[w.id] === "done" ||
                          archiveStatus[w.id] === "exists"
                        }
...
function OrdersTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<Order[]>(`${API}/admin/orders`, token);
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1 flex-wrap">
                  ...
                  {o.occasion === "gift" && (
                    <span className="text-[9px] tracking-widest uppercase text-[#2D2926] border border-border/30 px-2 py-0.5">
                      Gift
                    </span>
                  )}
                </div>
                <p className="text-xs text-[#2D2926]">
                  {o.phone}&nbsp;·&nbsp;{o.qty} {o.qty === 1 ? "box" : "boxes"}
                  &nbsp;·&nbsp;₹{o.totalAmount.toLocaleString("en-IN")}
                </p>
                <p className="text-xs text-[#2D2926] mt-0.5 leading-relaxed">
                  {o.address}
                </p>
                {o.giftMessage && (
                  <p className="text-xs text-[#2D2926] mt-0.5 italic">
                ...
                <p className="text-[10px] text-[#2D2926] mt-1">
...
const SITE_MODES = [
  {
    ...
    label: "Bake Day",
    description:
      "Full menu visible. Order Now buttons active. Orders open for the current bake window.",
  },
  {
    id: "popup",
    label: "Pop-Up Mode",
    description:
      "Hides the order form. Shows: 'We are popping-up this week! Online orders are closed, but we'll be back next bake day. Stay tuned!'",
  },
  {
    id: "maintenance",
    label: "Maintenance",
    description:
      "Shows a holding message: 'Baking in progress — check back soon.'",
  },
  {
    id: "chef_on_break",
    label: "Chef on Break",
    description:
      "Shows a warm, personal pause message: the chef is recharging and will return inspired. Orders are hidden.",
  },
] as const;

function SiteModeTab({ token }: { token: string }) {
  const { data: settings, loading } = useAdminFetch<Setting[]>(
    `${API}/admin/settings`,
    token,
  );
  ...
  return (
    <div>
      ...
      <p className="text-xs text-[#2D2926] mb-6 leading-relaxed">
        Controls what visitors see on the homepage and Pre-order page. Changes
      ...
      {loading ? (
        ...
        <>
            ...
            {SITE_MODES.map((mode) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-4 h-4 rounded-full border-2 flex-shrink-0 mt-0.5 transition-colors ${selected === mode.id ? "border-accent bg-accent" : "border-border/50"}`}
                  />
                  <div>
                    <p
                      className={`text-sm font-medium mb-1 ${selected === mode.id ? "text-foreground" : "text-[#2D2926]"}`}
                    >
                      {mode.label}
                    </p>
          ...
          <div className="border border-border/25 p-4 mb-6 bg-muted/20">
            <p className="text-[10px] tracking-[0.18em] uppercase font-medium text-[#2D2926] mb-1">
              Sold Out — Automatic
            </p>
            <p className="text-xs text-[#2D2926] leading-relaxed">
              When confirmed + pending orders for the active bake window reach
              the Max Boxes limit, the site automatically shows a sold-out
              message. No manual action needed.
            </p>
          </div>
          <button
            onClick={handleSave}
...
const DEFAULT_SETTINGS: { key: string; label: string; placeholder: string }[] =
  [
    {
      key: "strip_message",
      label: "Info Strip Message (overrides auto-generated)",
      placeholder: "Leave blank to auto-generate from live bake window",
    },
    {
      key: "strip_enabled",
      label: "Info Strip Enabled (true/false)",
      placeholder: "true",
    },
    {
      key: "delivery_zone",
      label: "Delivery Zone",
      placeholder: "Free delivery within 7km of HSR Layout, Bengaluru",
    },
    {
      key: "upi_id",
      label: "UPI ID (your VPA, e.g. name@bank)",
      placeholder: "yourname@bank",
    },
    { key: "gift_price", label: "Gifting Box Price (₹)", placeholder: "1299" },
    {
      key: "gift_original_price",
      label: "Gifting Box Original Price / Strikethrough (₹)",
      placeholder: "1999",
    },
  ];

const PRODUCT_LIMIT_SETTINGS: {
  key: string;
  label: string;
  placeholder: string;
}[] = [
  {
    key: "max_wandering_boxes",
    label: "Wandering Box — available today (0 = sold out)",
    placeholder: "15",
  },
  {
    key: "max_small_boxes",
    label: "Mini Wandering Box — available today (0 = sold out)",
    placeholder: "99",
  },
  {
    key: "max_sourdough_boules",
    label: "Artisanal Sourdough Boule — available today (0 = sold out)",
    placeholder: "99",
  },
];
...
function SettingsTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<Setting[]>(`${API}/admin/settings`, token);
  ...
  return (
    <div>
      ...
      <div className="mb-10">
        <p className="text-[10px] tracking-[0.28em] uppercase font-medium text-[#2D2926] mb-1">
          Pre-order Product Limits
        </p>
        <p className="text-xs text-[#2D2926] mb-5 leading-relaxed">
          Set the quantity available to customers right now. Set to 0 to show as
          sold out on the Pre-order page.
        </p>
        {loading ? (
          <p className="text-xs text-[#2D2926]">Loading…</p>
        ) : (
      ...
      <p className="text-xs text-[#2D2926] mb-5 leading-relaxed">
        The info strip message auto-generates from the live bake window.
...
function LaunchItemsPanel({
  ...
  function getImageSrc(imageFilename: string | null): string | null {
    ...
    if (imageFilename.startsWith("/objects/"))
      return `${API.replace("/api", "")}/api/storage${imageFilename}`;
              ...
              <div className="flex-shrink-0">
                {getImageSrc(item.imageFilename) ? (
                  ...
                  <div
                    ...
                    style={{ background: "rgba(15,36,25,0.06)" }}
                  >
...
function ArchiveTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<Launch[]>(`${API}/admin/launches`, token);
  const [creating, setCreating] = useState(false);
  const [form, setForm] = useState({
    title: "",
    bakeDate: "",
  ...
  useEffect(() => {
    if (launches) {
      setOpenItems((prev) => {
        ...
        launches.forEach((l) => {
          if (l.items.length === 0) autoOpen[l.id] = true;
        });
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#2D2926] mt-1">
            Each launch is a bake day. Items inside show on the public archive
            page with ratings.
          </p>
        </div>
        <button
        ...
        {creating && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Bake Day #1 — March"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={`auto: ${form.title ? slugify(form.title) : "bake-day-1"}`}
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="Optional note shown on archive"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
            </div>
            <div className="flex gap-3">
              <button
                onClick={handleCreate}
                disabled={saving || !form.title || !form.bakeDate}
                className="text-xs tracking-[0.18em] uppercase font-medium px-6 h-9 bg-accent text-accent-foreground hover:bg-accent/90 transition-all disabled:opacity-40"
              >
                {saving ? "Saving…" : "Create"}
              </button>
      ...
      {loading ? (
        ...
        <p className="text-xs text-[#2D2926]">
          No launches yet. Create one to start building the archive.
        </p>
      ) : (
        <div className="divide-y divide-border/30">
          {launches.map((l) => (
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#2D2926] mt-0.5">
                    {new Date(l.bakeDate + "T00:00:00").toLocaleDateString(
                      "en-IN",
                      { day: "numeric", month: "long", year: "numeric" },
                    )}
                    &nbsp;·&nbsp;{l.items.length}{" "}
                    {l.items.length === 1 ? "item" : "items"}
                    {l.notes && (
                      <>
                ...
                <button
                  onClick={() => {
                    setEditId(editId === l.id ? null : l.id);
                    setEditForm({
                      title: l.title,
                      bakeDate: l.bakeDate,
                      slug: l.slug,
                ...
                {editId === l.id && (
                  ...
                  >
                    <div className="mt-3 p-4 border border-border/30 bg-muted/20 grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {(["title", "bakeDate", "slug", "notes"] as const).map(
                        (field) => (
                          <div key={field}>
                            <label className="text-[10px] tracking-[0.2em] uppercase text-[#2D2926] block mb-1">
                              {field === "bakeDate"
                                ? "Bake Date"
...
function RecipesTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<AdminRecipe[]>(`${API}/admin/recipes`, token);
  const [creating, setCreating] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [form, setForm] = useState({
    title: "",
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#2D2926] mt-1">
            Manage recipes shown on the public Recipes page. Tags are
            comma-separated.
          </p>
        </div>
        {!isEditing && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Inspired by Paragon Restaurant, Kozhikode"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Kerala, Seafood, Curry"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              <div>
          ...
          {recipes.map((r) => (
            <div key={r.id} className="py-4 flex items-start gap-4">
              {r.imageFilename ? (
                <img
                  src={
                    r.imageFilename.startsWith("/objects/")
                      ? `/api/storage${r.imageFilename}`
                      : `/images/${r.imageFilename}`
                  }
                  alt={r.title}
              ...
              <div className="flex-1 min-w-0">
                ...
                {r.subtitle && (
                  <p className="text-xs text-[#2D2926]">{r.subtitle}</p>
                )}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <label className="text-[10px] tracking-widest uppercase px-3 h-8 border border-border/40 text-[#2D2926] hover:text-foreground hover:border-foreground/50 transition-all cursor-pointer flex items-center">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      e.target.files?.[0] &&
                      handleImageUpload(r.id, e.target.files[0])
                    }
                  />
                </label>
                <button
...
function ReviewsTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<AdminTestimonial[]>(`${API}/admin/testimonials`, token);
  const [creating, setCreating] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [form, setForm] = useState({
    authorName: "",
  ...
  async function handleDelete(id: number) {
    ...
    const res = await apiCall(`${API}/admin/testimonials/${id}`, {
      method: "DELETE",
      headers: { "x-admin-token": token },
    });
    if (!res.ok) {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#2D2926] mt-1">
            Manage customer reviews shown on the homepage. Toggle visibility
            without deleting.
          </p>
        </div>
        {!isEditing && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <div className="flex items-center gap-3 pt-5">
                ...
                <label
                  ...
                  className="text-[10px] tracking-widest uppercase text-[#2D2926] cursor-pointer"
                >
                  Visible on site
                </label>
              </div>
        ...
        <div className="divide-y divide-border/30">
          {testimonials.map((t) => (
            <div key={t.id} className="py-4 flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-serif text-sm text-foreground italic leading-snug mb-1">
                  "{t.body}"
                </p>
...
function BakeryAddOnsTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<AdminAddon[]>(`${API}/admin/bakery-addons`, token);
  const [creating, setCreating] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [form, setForm] = useState({
    title: "",
  ...
  function loadIntoForm(a: AdminAddon) {
    setForm({
      ...
      preorderCloseDate: a.preorderCloseDate ?? "",
    });
  }
  ...
  function buildBody() {
    return {
      ...
      preorderCloseDate: form.preorderCloseDate || null,
    };
  }

  async function handleCreate() {
    setSaving(true);
    const res = await apiCall(`${API}/admin/bakery-addons`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
  ...
  async function handleUpdate() {
    if (!editId) return;
    setSaving(true);
    const res = await apiCall(`${API}/admin/bakery-addons/${editId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
      body: JSON.stringify(buildBody()),
    });
  ...
  async function handleDelete(id: number) {
    ...
    const res = await apiCall(`${API}/admin/bakery-addons/${id}`, {
      method: "DELETE",
      headers: { "x-admin-token": token },
    });
    if (!res.ok) {
  ...
  async function handleToggle(a: AdminAddon) {
    const res = await apiCall(`${API}/admin/bakery-addons/${a.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#2D2926] mt-1">
            Standalone products available alongside the bake window. Displayed
            on the Bakery page.
          </p>
        </div>
        {!isEditing && (
        ...
        {isEditing && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Artisanal Sourdough Boule"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              <div className="sm:col-span-2">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      pricePaise: Number(e.target.value),
                    }))
                  }
              ...
              <div>
                ...
                <input
                  type="date"
                  value={form.preorderCloseDate}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      preorderCloseDate: e.target.value,
                    }))
                  }
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              ...
              <div className="sm:col-span-2">
                <label className="text-[10px] tracking-widest uppercase text-[#2D2926] block mb-1">
                  Image URL (or /images/filename.jpg)
                </label>
                <input
                  value={form.imageUrl}
                  onChange={(e) =>
                  ...
                  placeholder="/images/addon-sourdough-boule.png"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={editId ? handleUpdate : handleCreate}
                disabled={saving || !form.title || !form.pricePaise}
                className="text-xs tracking-[0.18em] uppercase font-medium px-6 h-9 bg-accent text-accent-foreground hover:bg-accent/90 transition-all disabled:opacity-40"
              >
                {saving ? "Saving…" : editId ? "Update" : "Create"}
              </button>
          ...
          {addons.map((a) => (
            <div key={a.id} className="py-4 flex items-start gap-4">
              {a.imageUrl ? (
                <img
                  src={
                    a.imageUrl.startsWith("/images/") ? a.imageUrl : a.imageUrl
                  }
                  alt={a.title}
                  className="w-12 h-12 object-cover rounded-sm flex-shrink-0"
                />
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[#2D2926]">
                  {formatPaise(a.pricePaise)}
                </p>
                {a.description && (
                  <p className="text-[10px] text-[#2D2926] mt-0.5 truncate">
                    {a.description}
                  </p>
                )}
              </div>
...
function JournalTab({ token }: { token: string }) {
  const {
    ...
    loading,
    refetch,
  } = useAdminFetch<AdminJournalEvent[]>(`${API}/admin/journal`, token);
  const [creating, setCreating] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [form, setForm] = useState({
    title: "",
  ...
  async function handleCreate() {
    setSaving(true);
    const res = await apiCall(`${API}/admin/journal`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
      body: JSON.stringify(buildBody()),
    });
  ...
  async function handleUpdate() {
    if (!editId) return;
    setSaving(true);
    const res = await apiCall(`${API}/admin/journal/${editId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
      body: JSON.stringify(buildBody()),
    });
  ...
  async function handleDelete(id: number) {
    ...
    const res = await apiCall(`${API}/admin/journal/${id}`, {
      method: "DELETE",
      headers: { "x-admin-token": token },
    });
    if (!res.ok) {
  ...
  async function handleTogglePublish(e: AdminJournalEvent) {
    const res = await apiCall(`${API}/admin/journal/${e.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "x-admin-token": token },
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#2D2926] mt-1">
            Pop-ups, events, and field notes shown on the public Journal page.
          </p>
        </div>
        {!isEditing && (
          <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Pop-Up at Koramangala Farmers Market"
                  className="w-full h-10 border border-border/50 bg-background text-foreground text-xs px-3 focus:outline-none focus:border-accent"
                />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                <label className="text-[10px] tracking-widest uppercase text-[#2D2926] block mb-1">
                  Image URLs (one per line, max 4)
                </label>
                <textarea
                  value={form.mediaUrlsRaw}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, mediaUrlsRaw: e.target.value }))
                  }
                  rows={3}
          ...
          {events.map((e) => (
            <div key={e.id} className="py-4 flex items-start gap-4">
              <div className="flex-1 min-w-0">
                ...
                {e.eventDate && (
                  <p className="text-[10px] text-[#2D2926]">{e.eventDate}</p>
                )}
                {e.body && (
                  <p className="text-[10px] text-[#2D2926] mt-0.5 truncate">
                    {e.body}
                ...
                <div className="flex gap-2 mt-1">
                  {e.mediaUrls && e.mediaUrls.length > 0 && (
                    ...
                    >
                      {e.mediaUrls.length} image
                      {e.mediaUrls.length > 1 ? "s" : ""}
                    </span>
                  )}
                  {e.embedUrl && (
                    <span
...
export default function Admin() {
  const [token, setToken] = useState(
    () => localStorage.getItem("wc_admin_token") ?? "",
  );
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-10">
          <div>
            <p className="text-[10px] tracking-[0.3em] uppercase text-[#2D2926] mb-1">
              Wandering Cocos
            </p>
          ...
          {authed && (
            <button
              ...
              className="text-[10px] tracking-[0.22em] uppercase text-[#2D2926] hover:text-foreground/60 transition-colors"
            >
              Sign out
            </button>
          )}
