    ...
    <div className="flex flex-col items-start gap-2 mt-3">
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <svg
              ...
              strokeWidth="1.5"
              style={{
                color: star <= active ? "hsl(150 40% 28%)" : "rgba(15,36,25,0.3)",
      ...
      <div className="flex items-baseline gap-2">
        {voteCount > 0 ? (
          <>
            <span
              ...
              style={{ fontSize: "0.95rem", color: "hsl(150 40% 28%)" }}
            >
              {avgStars.toFixed(1)}
            </span>
            <span
        ...
        {voted && (
          <span
            className="text-[10px] tracking-[0.12em] uppercase"
            style={{ color: "hsl(150 40% 28%)", opacity: 0.7 }}
          >
            · Your vote: {voted}★
          </span>
        )}
    ...
    >
      {imageSrc ? (
        ...
        >
          <span
            className="font-serif italic"
            style={{ fontSize: "1.4rem", color: "rgba(15,36,25,0.2)" }}
          >
      ...
      <div className="flex-grow min-w-0">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p
              className="text-[9px] tracking-[0.22em] uppercase font-medium mb-1"
              style={{ color: "rgba(15,36,25,0.35)" }}
            >
              {String(index + 1).padStart(2, "0")}
            </p>
            ...
            {item.description && (
              <p
                ...
                style={{ fontSize: "0.78rem", color: "rgba(15,36,25,0.55)" }}
              >
                {item.description}
              </p>
            )}
...
export default function TheArchives() {
  ...
  const handleRate = useCallback(async (itemId: number, stars: number) => {
    ...
    try {
      const res = await fetch(`/api/archive/items/${itemId}/rate`, {
        method: "POST",
      ...
      setLaunches((prev) =>
        prev.map((launch) => ({
          ...launch,
          items: launch.items.map((item) =>
            item.id === itemId ? { ...item, avgStars, voteCount } : item
          ),
        }))
      );
    } catch {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "hsl(38 25% 96%)" }}>
      ...
      <main className="flex-grow max-w-3xl mx-auto w-full px-6 md:px-10 pt-20 pb-24">
        ...
        <div className="mb-16">
          <p
            className="text-[9px] tracking-[0.36em] uppercase font-medium mb-5"
            style={{ color: "rgba(15,36,25,0.35)" }}
          >
            Wandering Cocos
          </p>
          <h1
            className="font-serif font-medium leading-none mb-5"
            style={{ fontSize: "clamp(2.4rem, 6vw, 4rem)", color: "#0f2419" }}
          >
            The Archives
          </h1>
          <p
            className="font-serif italic leading-relaxed max-w-lg"
        ...
        {!loading && !error && launches.map((launch) => {
          ...
          return (
            <section key={launch.id} className="mb-16">
              {/* Launch header */}
              <div
                className="flex items-center gap-5 mb-2 pb-4"
                style={{ borderBottom: "2px solid #0f2419" }}
              >
                <div>
                  <h2
                    className="font-serif font-medium"
                    style={{ fontSize: "1.35rem", color: "#0f2419" }}
                  >
                    {launch.title}
                  </h2>
                  <p
                ...
                {launch.notes && (
                  <p
                    className="font-serif italic ml-auto text-right"
                    style={{ fontSize: "0.82rem", color: "rgba(15,36,25,0.45)", maxWidth: "200px" }}
                  >
                    {launch.notes}
                  </p>
                )}
